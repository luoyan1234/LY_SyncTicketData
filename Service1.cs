...
namespace LY_SyncTicketData
{
    public partial class Service1 : ServiceBase
    {
        ...
        List<OrderInfo> listAdd = new List<OrderInfo>();//待推送到中心队列
        //List<OrderSimple> listChangePlanDate = new List<OrderSimple>();//待改签订单队列
        //List<OrderSimple> listReturnCard = new List<OrderSimple>();//待退票队列
        List<OrderSimple> listChangeOrderStatus = new List<OrderSimple>();//等更新状态订单队列
                ...
                try
                {
                    ...
                    if (ds != null && ds.Rows.Count > 0)
                    {
                        foreach (DataRow dr in ds.Rows)//向中心添加订单队列
                        {
                            ...
                            order.Ticketcode = dr["QRCode"].ToString();
